package com.delivery.deliverycore.config.log;
...
public class LoggingAspect {
    ...
    @Around("execution(* com.*.*.infrastructure.kafka.DeliveryKafkaProducer.*(..))")
    public Object aroundKafkaProducerMethodExecution(ProceedingJoinPoint joinPoint) throws Throwable {
        Result result = getResult(joinPoint);
        return loggingProcess(joinPoint, result);
    }

    @Around("execution(* com.delivery.deliveryconsumer.DeliveryConsumer.*(..))")
